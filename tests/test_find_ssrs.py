...
from massgenotyping.find_ssrs import (find_ssrs, gen_motif_classes, get_longest_RepData,
                                      get_motif_class, motif_set, remove_gaps, rotate,
                                      unfold_rep_seq)
...
class TestMotifSet(unittest.TestCase):
    def test_motif_set(self):
        ans = ["AAG", "AGA", "GAA", "CTT", "TTC", "TCT"]
        for x, y in zip(motif_set("AAG"), ans):
            self.assertEqual(x, y)


class TestGenMotifClasses(unittest.TestCase):
    def test_gen_motif_classes(self):
        ans = ["AAC", "AAG", "AAT", "ACC", "ACG", "ACT", "AGC", "AGG", "ATC", "CCG"]
        for x, y in zip(gen_motif_classes(3, 3), ans):
            self.assertEqual(x, y)


class TestGetMotifClass(unittest.TestCase):
    def test_get_motif_class(self):
        for test_string in ["AAG", "AGA", "GAA", "CTT", "TTC", "TCT"]:
            self.assertEqual(get_motif_class(test_string), "AAG")


class TestRemoveGaps(unittest.TestCase):
    def test_remove_gaps(self):
        result = remove_gaps("ATT--AAT")
        self.assertEqual(result, ("ATTAAT", [0, 1, 2, 5, 6, 7]))


class TestFindSSRs(unittest.TestCase):
...
class TestUnfoldRepSeq(unittest.TestCase):
...
class TestGetLongestRepData(unittest.TestCase):
    def test_get_longest_RepData(self):
